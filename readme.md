...
Z: 切換飛行模式\
V: 開始/停止錄影 (存到 recordings/，同時產生乾淨畫面、疊字畫面與飛行紀錄 CSV)

手部鎖定操作
F: 鍵切換鎖定模式
R: 鍵重新鎖定新目標

環繞操作 (HAND TRACKER 模式)
O: 鍵循環切換環繞方向，每按一次前進一個狀態\
　　關閉 -> 逆時針 (俯視，無人機往右橫移) -> 順時針 (往左橫移) -> 關閉

環繞中會自動拉近到較短的距離 (平時跟隨較遠)，切換飛行模式時會自動關閉環繞。
